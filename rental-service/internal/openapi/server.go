...
func (s *Server) GetUserRentals(c echo.Context, params openapi.GetUserRentalsParams) error {
	rents, err := s.rentalLogic.GetUserRentals(c.Request().Context(), params.UserName)
	if err != nil {
...
func (s *Server) Create(c echo.Context, params openapi.CreateParams) error {
	...
	logicReq, err := toRentCreateRequest(req, params.UserName)
	if err != nil {
...
func (s *Server) Cancel(c echo.Context, rentalUid openapi_types.UUID, params openapi.CancelParams) error {
	err := s.rentalLogic.Cancel(c.Request().Context(), rentalUid, params.UserName)
	if err != nil {
...
func (s *Server) Get(c echo.Context, rentalUid openapi_types.UUID, params openapi.GetParams) error {
	rent, err := s.rentalLogic.Get(c.Request().Context(), rentalUid, params.UserName)
	if err != nil {
...
func (s *Server) Finish(c echo.Context, rentalUid openapi_types.UUID, params openapi.FinishParams) error {
	err := s.rentalLogic.Finish(c.Request().Context(), rentalUid, params.UserName)
	if err != nil {
